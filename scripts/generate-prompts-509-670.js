#!/usr/bin/env node
/**
 * 从原项目 README.md 提取 509-670 案例，生成 prompts-509-670.json
...
async function main() {
  console.log(`📊 开始提取案例 ${START_ID}-${END_ID}...\n`);
  ...
  while ((match = caseRegex.exec(readme)) !== null) {
    ...
    } else if (fs.existsSync(path.join(imageBasePath, `${id}.jpg`))) {
  ...
  // 写入文件
  const outputPath = path.join(__dirname, `../data/prompts-${START_ID}-${END_ID}.json`);
  fs.writeFileSync(outputPath, JSON.stringify(output, null, 2), 'utf-8');

  console.log(`✅ 已生成文件: data/prompts-${START_ID}-${END_ID}.json`);
